...
Class Actions extends DBConnection{
    ...
    function login(){
        ...
        $qry = $result ? $result->fetch_assoc() : null;
    ...
    extract($_POST);
    $data = "";
    $cols = []; // Initialize $cols and $vals
    $vals = [];
    
    // Escape and prepare the data for insertion or update
    foreach ($_POST as $k => $v) {
        if (!in_array($k, array('id'))) {
            $v = trim($v);
            $v = $this->escapeString($v);  // Ensure escapeString() is defined or use mysqli_real_escape_string
    ...
    // SQL statement based on whether it's an insert or update
    if (empty($id)) {
    ...
    $check = $this->query("SELECT COUNT(department_id) AS count FROM `department_list` WHERE `name` = '{$name}'" . 
                          ($id > 0 ? " AND department_id != '{$id}'" : ""))
               ->fetch_assoc()['count'];
    ...
    if ($check > 0) {
        ...
        if ($save) {
            ...
            $resp['error'] = $this->lastErrorMsg();  // Capture last error message
            $resp['sql'] = $sql;  // Log the SQL query for debugging
        }
    }
    ...
    function save_designation(){
        ...
        @$check= $this->query("SELECT COUNT(designation_id) as count from `designation_list` where `name` = '{$name}' ".($id > 0 ? " and designation_id != '{$id}'" : ""))->fetch_assoc()['count'];
        if(@$check> 0){
            $resp['status'] ='failed';
    ...
    function save_vacancy(){
        ...
        @$check= $this->query("SELECT COUNT(vacancy_id) as count from `vacancy_list` where `designation_id` = '{$designation_id}' and status = 1 ".($id > 0 ? " and vacancy_id != '{$id}'" : ""))->fetch_assoc()['count'];
        if(@$check> 0){
            $resp['status'] ='failed';
    ...
    $save = $this->query($sql);
    if($save){
        $applicant_id = $this->query("SELECT LAST_INSERT_ID()")->fetch_row()[0];
        
        if(isset($company_name)){
            $data = "";
            foreach($company_name as $k => $v){
                $still_active[$k] = isset($still_active[$k]) ? 1 : 0;
                if(!empty($data)) $data .= ", ";
                $data .= "('{$applicant_id}', '{$company_name[$k]}', '{$from[$k]}', '{$to[$k]}', '{$position[$k]}', '{$brief_info[$k]}', '{$still_active[$k]}')";
            }
            $this->query("INSERT INTO `employment` (`applicant_id`, `company_name`, `from`, `to`, `position`, `brief_info`, `still_active`) VALUES {$data}");
        }
        ...
        if(isset($school_name)){
            ...
            $this->query("INSERT INTO `educational` (`applicant_id`, `school_name`, `from`, `to`, `level`, `school_address`) VALUES {$data}");
        }
        ...
        if(isset($skills)){
            ...
            $this->query("INSERT INTO `skills` (`applicant_id`, `skill`) VALUES {$data}");
        }
