# HRIMS - Human Resource Information Management System

## Project Overview
HRIMS (Human Resource Information Management System) is a web-based application built with pure PHP and Bootstrap that manages human resource operations, including employee records, attendance tracking, leave management, and personal data sheet (PDS) generation.

## Technology Stack

### Backend
- **PHP 7.4+** (Pure PHP, no frameworks)
- **MySQL** Database
- **PDO** for database operations

### Frontend
- **Bootstrap 5** (CSS framework)
- **JavaScript** (Vanilla JS)
- **jQuery** (for interactive components)
- **Chart.js** (for data visualization)

## Changelog

### Version 1.0.0
- Initial release
- Employee management system
- PDS generation module
- Attendance tracking
- Leave management system

---
**Note:** This is a pure PHP project without any PHP frameworks. All code is written in procedural/OOP PHP with Bootstrap for frontend styling.
